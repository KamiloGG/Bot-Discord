    ...
    execute(client, message, args) {
        ...
        if (args.join(" ").toLowerCase() === 'queue') {
            if (client.player.getQueue(message).loopMode) {
                ...
                return message.channel.send(`${client.emotes.success} - Tryb powtarzania ** włączony ** cała kolejka będzie powtarzana w nieskończoność!`);
            };
        } else {
            if (client.player.getQueue(message).repeatMode) {
                ...
                return message.channel.send(`${client.emotes.success} - Tryb powtarzania ** włączony ** bieżąca muzyka będzie powtarzana w nieskończoność!`);
            };
        };
    },
};